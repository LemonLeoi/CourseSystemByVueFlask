...
def get_data_from_db():
    ...
    # 查询学生信息和成绩
    query = """
    SELECT s.student_id, s.gender, s.class, s.grade, g.exam_type, g.subject, g.score
    FROM students s
    JOIN student_grades g ON s.student_id = g.student_id
    WHERE g.exam_type IN ('期中考试', '期末考试')
    """
...
def analyze_student_grades():
    print("正在从数据库获取学生成绩数据...")
    data = get_data_from_db()
    print(f"获取到 {len(data)} 条成绩记录")
    
    print("\n正在预处理数据...")
    processed_data, attr_names = preprocess_data(data)
    
    # 选择特征和目标变量
    # 特征: 性别(1), 班级(2), 年级(3), 考试类型(4), 学科(5)
    # 目标: 分数范围(6)
    features = set(range(1, 6))  # 选择除了学号和分数范围之外的所有特征
    target_index = 6
    
    print("\n正在构建决策树...")
    tree = build_tree(processed_data, features, T=target_index, algo="C4.5")
    
    print("\n决策树分析结果:")
    print_tree(tree, attr_names=attr_names)
