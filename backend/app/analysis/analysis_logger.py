...
class AnalysisLogger:
    ...
    def __init__(self, log_dir=None):
        ...
        Args:
            log_dir: 日志存储目录，默认为项目根目录下的logs文件夹
        """
        if log_dir is None:
    ...
    def _get_default_log_dir(self):
        ...
        try:
            ...
            return os.path.join(os.path.expanduser('~'), 'A_Course', 'logs')
    
    def _init_log_file(self):
        """初始化日志文件（在创建目录后调用）"""
        self.log_file = os.path.join(self.log_dir, f'analysis_{datetime.datetime.now().strftime("%Y%m%d")}.json')
        self.logs = []
