...
def calculate_normalized_average(class_name, grade, subjects=None):
    """计算标准化平均分（得分率）
    
    Args:
        class_name: 班级名称
        grade: 年级
        subjects: 学科列表（可选，为None时计算所有学科）
    
    Returns:
        dict: 包含原始分数和标准化分数的统计数据
    """
    print(f"calculate_normalized_average 被调用: class_name={class_name}, grade={grade}, subjects={subjects}")
    ...
    try:
        if subjects:
            ...
            for subject in subjects:
                try:
                    stats = GradeDataAccess.get_class_subject_statistics(class_name, grade, subject)
                    if stats:
                        subject_stats.update(stats)
                except Exception as e:
                    print(f"获取学科 {subject} 统计数据时出错: {e}")
                    continue
        else:
            subject_stats = GradeDataAccess.get_class_subject_statistics(class_name, grade)
    except Exception as e:
        print(f"获取学科统计数据时出错: {e}")
        return {"error": f"获取班级成绩数据失败: {str(e)}"}
    
    print(f"获取到的 subject_stats: {list(subject_stats.keys()) if subject_stats else '空'}")
    ...
    for subject, stats in subject_stats.items():
        try:
            ...
            # 计算得分率（标准化分数）
            raw_avg = stats['average_score']
            normalized_score = GradeDataAccess.calculate_score_rate(raw_avg, subject)
            
            normalized_result[subject] = {
                'raw_score': raw_avg,
                'full_score': full_score,
                'normalized_score': normalized_score,  # 得分率（0-100）
                'student_count': stats['student_count'],
                'excellent_rate': stats['excellent_rate'],
                'pass_rate': stats['pass_rate']
            }
            
            total_raw_avg += raw_avg
            total_normalized_avg += normalized_score
            subject_count += 1
            print(f"学科 {subject} 处理成功: 原始 {raw_avg}, 标准化 {normalized_score}%, 满分 {full_score}")
        except Exception as e:
    ...
    result = {
        ...
        'subject_details': normalized_result,
        'overall': {
            'raw_average': round(total_raw_avg / subject_count, 2) if subject_count > 0 else 0,
            'normalized_average': round(total_normalized_avg / subject_count, 2) if subject_count > 0 else 0
        }
    }
