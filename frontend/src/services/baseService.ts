...
// 基础CRUD服务接口
export interface IBaseService<T extends Record<string, any>, CreateDto = T, UpdateDto = Partial<T>> {
  // 获取所有数据
  getAll(params?: Record<string, any>): Promise<ApiResponse<T[]>>;
  
  // 分页获取数据
  getPaginated(page: number, pageSize: number, params?: Record<string, any>): Promise<ApiResponse<PaginatedResponse<T>>>;
...
// 基础服务实现（使用模拟数据）
export abstract class BaseService<T extends Record<string, any>, CreateDto = T, UpdateDto = Partial<T>> implements IBaseService<T, CreateDto, UpdateDto> {
  protected data: T[] = [];
  protected nextId = 1;
  ...
  // 分页获取数据
  async getPaginated(page: number, pageSize: number, params?: Record<string, any>): Promise<ApiResponse<PaginatedResponse<T>>> {
    try {
...
// 错误响应
export function errorResponse<T>(code: number, message: string, data: T = {} as T): ApiResponse<T> {
