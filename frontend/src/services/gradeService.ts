...
export class GradeService {
  ...
  // 学科对比分析改进API
  async compareSubjectsNormalized(className: string, grade: string, subjects?: string[]): Promise<NormalizedSubjectComparisonResponse> {
    const params = new URLSearchParams();
    ...
    return await fetchApi<NormalizedSubjectComparisonResponse>(`/analysis/compare-subjects-normalized?${params.toString()}`);
  }

  async getSubjectConfig(): Promise<SubjectConfigResponse> {
    return await fetchApi<SubjectConfigResponse>('/analysis/subject-config');
  }
...
export interface NormalizedSubjectDetail {
  raw_score: number;
  full_score: number;
...
export interface NormalizedSubjectComparisonResponse {
  success: boolean;
  data: {
    class_info: {
      class_name: string;
    ...
    subject_details: Record<string, NormalizedSubjectDetail>;
    overall: {
      raw_average: number;
