        ...
        >
          ...
          <div class="template-selector">
            ...
            <div class="template-groups">
              <div v-for="group in templateOptions" :key="group.name" class="template-group">
                <span class="group-name">{{ group.name }}</span>
                <button 
                  v-for="(option, index) in group.options" 
                  :key="index"
                  class="template-btn"
                  @click="applyTemplate(option)"
                >
                  {{ option[0] }}+{{ option[1] }}
                </button>
              </div>
          ...
          <div class="subject-comparison-selector">
            ...
            <button 
              class="compare-subjects-btn" 
              @click="compareSubjectsNormalized"
              :disabled="isComparingNormalized"
            >
              {{ isComparingNormalized ? '分析中...' : '标准化对比' }}
            </button>
          </div>
          
          <!-- 标准化对比结果 -->
          <div v-if="normalizedComparisonData" class="normalized-comparison-result">
            <h4>标准化对比（得分率）</h4>
            <div class="normalized-toggle">
              <button 
                :class="['toggle-btn', displayModeNormalized === 'raw' ? 'active' : '']"
                @click="displayModeNormalized = 'raw'"
              >原始分数</button>
              <button 
                :class="['toggle-btn', displayModeNormalized === 'normalized' ? 'active' : '']"
                @click="displayModeNormalized = 'normalized'"
              >得分率(%)</button>
            </div>
            <div class="normalized-chart-container">
              <div class="normalized-bar">
                <div class="bar-item">
                  <span class="bar-label">{{ normalizedComparisonData.subjects[0] }}</span>
                  <div class="bar-wrapper">
                    <div 
                      class="bar-fill" 
                      :style="{ width: (displayModeNormalized === 'raw' ? normalizedComparisonData.raw_scores[0] / normalizedComparisonData.full_scores[0] * 100 : normalizedComparisonData.normalized_scores[0]) + '%' }"
                      :class="['bar-1']"
                    ></div>
                  </div>
                  <span class="bar-value">
                    {{ displayModeNormalized === 'raw' ? normalizedComparisonData.raw_scores[0] : normalizedComparisonData.normalized_scores[0].toFixed(1) }}
                    {{ displayModeNormalized === 'raw' ? '/' + normalizedComparisonData.full_scores[0] : '%' }}
                  </span>
                </div>
                <div class="bar-item">
                  <span class="bar-label">{{ normalizedComparisonData.subjects[1] }}</span>
                  <div class="bar-wrapper">
                    <div 
                      class="bar-fill" 
                      :style="{ width: (displayModeNormalized === 'raw' ? normalizedComparisonData.raw_scores[1] / normalizedComparisonData.full_scores[1] * 100 : normalizedComparisonData.normalized_scores[1]) + '%' }"
                      :class="['bar-2']"
                    ></div>
                  </div>
                  <span class="bar-value">
            ...
            <div class="normalized-summary">
              <div class="summary-item">
                <span class="summary-label">得分率差异:</span>
                <span class="summary-value" :class="normalizedComparisonData.diff >= 0 ? 'positive' : 'negative'">
                  {{ normalizedComparisonData.diff >= 0 ? '+' : '' }}{{ normalizedComparisonData.diff.toFixed(1) }}%
                </span>
              </div>
              <div class="normalized-note">
                <strong>说明：</strong>得分率 = 原始分数 / 学科满分 × 100%，消除了不同学科满分差异的影响，使跨学科比较更具科学性。
              </div>
            </div>
          </div>
        </CollapsibleSection>
...
export default {
  ...
  setup() {
    ...
    // 按钮loading状态
    const isComparingNormalized = ref(false);
    ...
    // 标准化对比相关数据
    const normalizedComparisonData = ref(null);
    const displayModeNormalized = ref('raw'); // 'raw' | 'normalized'
    
    // 预设模板选项
    const templateOptions = [
      { name: '理科组合', options: [['数学', '物理'], ['数学', '化学'], ['物理', '化学']] },
      { name: '文科组合', options: [['语文', '历史'], ['语文', '政治'], ['历史', '政治']] },
      { name: '跨科组合', options: [['数学', '语文'], ['物理', '历史']] }
    ];
    ...
    // 学科标准化对比（使用原始分数 × 得分率系数）
    const compareSubjectsNormalized = async () => {
      console.log('标准化对比按钮被点击', { s1: subjectCompare1.value, s2: subjectCompare2.value, className: className.value });
      ...
      isComparingNormalized.value = true;
      try {
        // 使用原始分数对比的API获取数据，然后在前端进行标准化转换
        const response = await gradeService.compareSubjects(className.value, [subjectCompare1.value, subjectCompare2.value]);
        
        console.log('原始分数对比响应:', response);
        
        if (response && response.base_subject) {
          // 获取原始分数
          const subject1Score = response.base_subject.average_score || 0;
          let subject2Score = subject1Score;
          let excellentDiff = 0;
          let passDiff = 0;
          
          if (response.comparisons && response.comparisons.length > 0) {
            const comparison = response.comparisons[0];
            subject2Score = subject1Score + comparison.average_diff;
            excellentDiff = comparison.excellent_diff;
            passDiff = comparison.pass_diff;
          }
          
          // 预设得分率系数（模拟标准化转换）
          // 语文、数学、英语满分150分，转换为百分制的系数为 100/150 ≈ 0.6667
          // 其他学科满分100分，系数为 1.0
          const scoreRateMap = {
            '语文': 0.6667,
            '数学': 0.6667,
          ...
          // 计算标准化分数（原始分数 × 得分率系数）
          const subject1Rate = scoreRateMap[subjectCompare1.value] || 1.0;
          const subject2Rate = scoreRateMap[subjectCompare2.value] || 1.0;
          
          const normalizedScore1 = Math.round(subject1Score * subject1Rate * 100) / 100;
          const normalizedScore2 = Math.round(subject2Score * subject2Rate * 100) / 100;
          
          // 构建标准化对比数据
          normalizedComparisonData.value = {
            subjects: [subjectCompare1.value, subjectCompare2.value],
            raw_scores: [subject1Score, subject2Score],
            normalized_scores: [normalizedScore1, normalizedScore2],
            full_scores: [
              subject1Rate === 0.6667 ? 150 : 100,
              subject2Rate === 0.6667 ? 150 : 100
            ],
            diff: Math.round((normalizedScore2 - normalizedScore1) * 100) / 100,
            excellent_diff: excellentDiff,
            pass_diff: passDiff,
            subject_details: {
              [subjectCompare1.value]: {
                raw_score: subject1Score,
                normalized_score: normalizedScore1,
                full_score: subject1Rate === 0.6667 ? 150 : 100
              },
              [subjectCompare2.value]: {
                raw_score: subject2Score,
                normalized_score: normalizedScore2,
                full_score: subject2Rate === 0.6667 ? 150 : 100
              }
            }
          };
          
          console.log('标准化数据设置成功:', normalizedComparisonData.value);
        } else {
          console.warn('响应数据格式异常:', response);
          alert('获取标准化对比数据失败，请检查数据是否完整');
        }
      } catch (error) {
        console.error('标准化学科对比失败:', error);
        alert(`标准化对比失败: ${error.message || '网络错误，请稍后重试'}`);
      } finally {
        isComparingNormalized.value = false;
      }
    };

    const applyTemplate = (subjects) => {
      subjectCompare1.value = subjects[0];
      subjectCompare2.value = subjects[1];
    };
    ...
    return {
      ...
      displayModeNormalized,
      compareSubjectsNormalized,
      isComparingNormalized,
      templateOptions,
      applyTemplate,
      // 分析类型和详细成绩数据
      analysisType,
...
/* 按钮样式扩展 */
.compare-subjects-btn.normalized-btn {
  background: linear-gradient(135deg, #0891b2, #06b6d4);
}

.compare-subjects-btn.normalized-btn:hover {
